        ...
        for (playerIndex, player) in players.enumerated() {
            ...
            if schema == "nickstats.match/22" || schema == compactSchema {
                ...
                for (index, slice) in slices.enumerated() {
                    let slicePath = "\(path).round_slices[\(index)]"
                    guard timingByRound[slice.round] != nil, seen.insert(slice.round).inserted else {
                        try invalid(slicePath, "Expected one slice per played, completed round.")
                    }
                    guard slice.buy == nil || ["pistol", "eco", "force", "full"].contains(slice.buy!),
                    ...
                    guard stats.rounds.played == 1, stats.rounds.won == (slice.result == "win" ? 1 : 0),
                          stats.kastRounds <= 1, stats.profile?.count == 16 else {
                        try invalid(slicePath, "Expected one played round, consistent result, and 16 profile counters.")
                    }
                }
                for side in PlayerSide.allCases {
                    let selected = slices.filter { $0.side == side }.map(\.stats)
                    let aggregate = player.sides[side]
                    guard selected.count == aggregate.rounds.played,
                          selected.reduce(0, { $0 + $1.combat.kills }) == aggregate.combat.kills,
                          selected.reduce(0, { $0 + $1.combat.deaths }) == aggregate.combat.deaths,
                          selected.reduce(0, { $0 + $1.combat.damage }) == aggregate.combat.damage else {
                        try invalid("\(path).round_slices", "Round slices must add up to the player's side totals.")
                    }
                }
            }
