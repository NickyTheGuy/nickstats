...
The API imports compact `nickstats.match/22` files into the normalized MySQL schema and exposes read-only match/player endpoints. Schemas 9 through 21 remain readable for existing stored matches; the browser still parses demos locally and uploads only the compact result. Run database migrations through 017 before deploying this backend.
