...
The Player view keeps multiple closable profiles open as tabs and stores the ten most recently viewed players in local browser storage. Its active profile section plus its map, result, side, economy, and round-result filters are shared by every open profile, so switching players preserves one directly comparable detailed view. Open profiles form the available player pool in **Graphs**: up to five independent careers can be selected for a general comparison under those same shared filters, whether or not those players shared matches. Distribution graphs compare each player's percentage of matches within common value ranges and report mean, median, standard deviation, range, and sample size. Their horizontal buckets and vertical scale are derived from the full open-player pool, so checking or unchecking a series does not reshape the existing graph; distributions default to grouped bars, retain a line display option, and provide a 4–24 bucket control for explicit granularity. Single-player trends can fall back to match order; independent multi-player trends wait for complete `played_at` coverage so unrelated timelines are not falsely aligned. Conditional Groups profiles continue to graph Included players under the current lineup condition.
...
The Demo Parser tab uses [`@deademx/cs2` 4.0.0](https://github.com/Igor-Losev/deadem/tree/v4.0.0/packages/cs2) in a Web Worker. It reads kills, deaths, assists, headshots, damage, trade kills and deaths, trade opportunities and attempts, damage- and flash-assisted kills, confirmed own-flash kills, enemies flashed, flash assists, flash effects and applied blind duration by thrower and victim, HE and fire grenade damage, openings, exact 1K–5K rounds, 1v1–1v5 clutch wins, team assignments, the map, and round winners from the demo event stream. CS2 exposes only one assister on a death event, so NickStats supplements that event with its independently tracked active flash sources: a damage assister and the teammate whose flash blinded the victim can both receive credit, while each player receives at most one assist for that kill. Opening statistics store the unique number of assisted opening kills plus overlapping damage- and flash-assisted breakdowns. An own-flash kill means the enemy was actively blinded by a flash thrown by the killer; it does not mean the killer blinded themselves. Flash relationships include enemies, teammates, and the thrower's own diagonal. Each `player_blind` effect contributes one flash and its reported blind duration. Enemy, teammate, and self effects and blind time are exposed independently in profiles, comparisons, graphs, and the match scoreboard, with totals, per-round, and per-flash views where applicable. It also tracks mirrored kill context: kills against blinded enemies and deaths while blind, wallbang and smoke kills/deaths, airborne kills and deaths to airborne killers, running kills/deaths, enemies caught with a grenade or knife active, and average horizontal killer speed. A Paul Kill/Death is one where the victim had a grenade or knife out at death or during the preceding 1.4 seconds. Bullshit Kills/Deaths uniquely count events where the killer was blind, airborne, or running, the kill was a wallbang or smoke kill, or the victim was caught for a Paul; overlapping contexts count once. Speed uses the pawn's networked velocity when present and otherwise derives it from horizontal position changes between demo packets. The scoreboard displays speed as a percentage of the held weapon's maximum movement speed, including scoped limits; values may exceed 100% after boosts or air movement. Grenade, lingering-fire, C4, and world kills are excluded because the killing weapon need not still be held. Raw units-per-second values remain in the JSON export. The JSON also records kills made while the attacker was blind and deaths to a blind attacker.

**Clawback Kills** are enemy kills made while the killer's team has fewer living players immediately before the kill. The same event is a **Bozo Death** for the victim, whose team had the man advantage immediately before the death. These counters are derived from the alive counts already stored with schema-10-and-later death events, so existing compatible matches do not need to be reparsed; schema-9 matches are excluded rather than contributing zeroes.

**Kill Stage** groups kills and deaths by the number of opponents alive immediately before the event, from five through one. Round-state context also separates even-state kills/deaths, advantage kills, outnumbered deaths, and cleanup events. A **Cleanup Kill** eliminates the last opponent while the killer's team still has at least three living players; the victim receives the mirrored Cleanup Death. These stats use the same schema-10-and-later death-event facts and likewise require no reparse.

Thrown-utility counts use the reliable `weapon_fire` event emitted when a grenade projectile is created. CS2's separate `grenade_thrown` event is not consistently present in demos and is not used for counting.

A running kill means the killer's horizontal speed exceeded **34% of the held weapon's maximum movement speed**, the point at which movement inaccuracy begins in the convention documented by [Leetify](https://leetify.com/blog/leetify-stats-glossary/). Separately, every speed-measured firearm kill is classified as **Moving** (above 1 Source 2 unit per second) or **Still** (at most 1 unit per second); the tolerance prevents tiny coordinate noise from labeling a stationary player as moving. The collapsed Kill Context column shows **Bullshit K-D**: unique kills/deaths where the killer was blind, airborne, or running, the kill was a wallbang or smoke kill, or the victim was caught for a Paul. A Paul is a kill/death where the victim had a grenade or knife out at death or within the preceding 1.4 seconds. A single event matching several conditions counts only once. Airborne and running appear in both Context, where they explain the composite, and Movement, where they sit alongside the detailed speed and movement-state statistics.
...
Quick comparison and the match scoreboard organize their section selectors like the profile: Overview, Opening, Trades, Rounds, Round state, Context, Movement, and Utility. Rounds contains Clutches, Kill rounds, and Objectives; Round state contains Man count, Kill stage, and Round timing; Context is reserved for visibility, cover, and readiness. The Player and Groups quick comparisons share the same selection and retain the compact Overview, Opening, and Rounds default. Quick comparison offers Totals, Per round, and Per match views; the single-match scoreboard offers Totals and Per round. A separate Per grenade option changes only meaningful utility yields, using the relevant HE, fire grenade, or flashbang count. Column labels spell out their active denominator. Only one column group expands at a time. Expanded groups with several detail sets place a cycle button directly in the group header, where repeated clicks move through choices such as Opening results, help received, help given, and flash context.

The Groups builder remembers its most recent roster and each player’s Include or Exclude role in local browser storage. Reopening the page restores those selections without caching match statistics; building the group still fetches current data from the API. Clear removes the saved roster.
...
Scoreboard headers are sortable within each team. Its expandable groups follow the player-profile order and color language: Combat, Opening, Trades, clutch/multikill/objective Rounds, Round timing, Context, Movement, and Utility. A visible section toolbar defaults to Combat, Opening, Trades, Clutches, and Utility, while **All** restores the complete scoreboard. Round timing shows average kill/death elapsed time plus Early, Mid, Late, and Post-plant K-D for the selected side. The expanded groups expose the corresponding single-match counters while keeping the default scoreboard compact. Expanding, collapsing, or hiding an unrelated section preserves the active sort; the sort clears only when its own column disappears. Composite headers cycle through their component statistics and then return to the original neutral order; single-stat headers toggle between that statistic and neutral. Favorable values sort first, so death-based penalty columns use fewer-first ordering.

The first trade-opportunity model is intentionally simple and transparent. A living teammate receives an opportunity when they are within 250 Source 2 game units of a teammate at the moment that teammate dies. This radius was calibrated against a known Leetify match result. A teammate outside that radius also receives a retroactive, proven opportunity if they damage or kill the killer within three seconds, fire a qualifying shot path near the killer, or are themselves killed by that same player. Damage, a qualifying shot, a kill, or dying to the killer proves an attempt; killing that player is a success. Success percentage uses attempts—not opportunities—as its denominator. A death is “tradeable” if at least one teammate met either rule, and it is counted only once regardless of how many teammates qualify. Its death-side attempt and success also count once even if multiple teammates act.
...
The three-second window determines whether a trade engagement can begin. A bullet path passing within 96 units of the original killer counts as an attempt even if the shot misses. Being killed by the original killer during that window also proves a failed attempt regardless of the teammate's initial distance. Once established, qualifying fire or damage from either side refreshes the engagement; it expires after a two-second lull. A kill of the original killer during that uninterrupted exchange remains a trade even when the fight lasts longer than three seconds. Bullet paths use the shooter, impact, and player positions recorded in the demo. They are an approximation and can still misidentify unusually aligned players without full map collision geometry.
...
Version 12 records each side's total equipment value and live player count at `round_freeze_end`, plus stable team identity and the pistol-round flag. Pistol rounds are regulation rounds 1 and 13 only. Non-pistol buys are derived per live player: Eco at $1,000 or less, Full Buy at $3,500 or more, and Force Buy between them. The $3,500 boundary recognizes a proper rifle-and-armor setup on either side without requiring a particular amount of utility. Storing the raw value and roster size lets those display thresholds change without another demo parse.

Version 18 stores sparse per-player statistic slices for the combination of the player's buy, the opposing team's buy, round result, and side. The Match, Player, Groups, Quick Comparison, and graph views therefore share one **Economy matchup** control with separate **Your buy** and **Enemy buy** rows. Matches parsed before version 18 do not contribute when an enemy-buy filter is active, rather than contributing false zeroes.
