...
Quick comparison can display every collapsible match-scoreboard section: Combat, Opening, Trades, Clutches, Context, Kill stage, Movement, Utility, Kill rounds, Objectives, and Round timing. Its visible section bar is shared between the Player and Groups views and retains the compact Combat, Opening, and Clutches default. Both quick comparison and the match scoreboard offer Totals and Rates views; Rates uses rounds by default and switches to per-grenade denominators where that is the meaningful comparison. Only one section expands at a time, and especially wide sections expose a second row of plain-language subgroups such as Opening results, help received, help given, and flash context.
