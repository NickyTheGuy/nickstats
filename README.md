...
Quick comparison can display every collapsible match-scoreboard section: Combat, Opening, Trades, Clutches, Context, Kill stage, Movement, Utility, Kill rounds, Objectives, and Round timing. Its visible section bar is shared between the Player and Groups views and retains the compact Combat, Opening, and Clutches default. Quick comparison offers Totals, Per round, and Per match views; the single-match scoreboard offers Totals and Per round. A separate Per grenade option changes only meaningful utility yields, using the relevant HE, fire grenade, or flashbang count. Column labels spell out their active denominator. Only one section expands at a time, and especially wide sections expose a second row of plain-language subgroups such as Opening results, help received, help given, and flash context.
