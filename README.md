...
A running kill means the killer's horizontal speed exceeded **34% of the held weapon's maximum movement speed**, the point at which movement inaccuracy begins in the convention documented by [Leetify](https://leetify.com/blog/leetify-stats-glossary/). The collapsed Kill Context column shows **Unfair K-D**: unique kills/deaths involving a blinded victim, penetration, smoke, an airborne killer, or a running killer. A single event matching several conditions counts only once. Expanding the group shows every component plus average killer speed percentage.
