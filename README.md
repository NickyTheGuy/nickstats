...
Side-specific ADR records each enemy-damage event directly into the attacker's live CT or T bucket. Its numerator and rounds-played denominator therefore cover the same side rather than reconstructing side damage from a later full-match total. A round stops accepting live combat events immediately at `round_end`, preventing post-round damage from entering CT/T totals after that round's denominator has already been finalized. Raw damage is included in each JSON player aggregate for validation.
