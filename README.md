...
Each player also receives a per-weapon ledger containing enemy kills, enemy health damage, `weapon_fire` events, and purchases. A shotgun firing event counts as one shot rather than one per pellet. FACEIT GOTV demos do not expose `item_purchase`, so purchases are inferred from buy-phase `item_pickup` events constrained by the player's recorded cash decrease and known item prices. Mid-round pickups and free spawn pistols are excluded, but dropped guns and unitemized equipment spending mean these counts should still be treated as estimates.
