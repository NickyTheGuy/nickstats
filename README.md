...
The diagnostic build also exposes **Download round diagnostics** after every successful parse. This separate JSON traces each completed round's end event, raw or inferred winner side, bomb and alive-state inference, frozen player-side assignments, participant set, awarded round wins, stable team winner, and delayed end events. It is intentionally excluded from match uploads and normal compact downloads.
